...
//@ts-ignore
const ProductCategoryImageDisplay = ({ setFileId, fileId, setError }) => {
  ...
  const handleImageUpload = async (
    ...
    if (event.target.files?.[0]) {
      ...
      if (result.success && result.fileId) {
        ...
        loadProductCategoryImage(result.fileId)
  ...
  async function loadProductCategoryImage(currentFileId: number) {
    //@ts-ignore
  ...
  useEffect(() => {
    if (fileId) {
      loadProductCategoryImage(fileId)
    }
  }, [fileId])
...
export default ProductCategoryImageDisplay
